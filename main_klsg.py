...
def main(json_path='options/train_sr_klsg.json'):
    '''
    ------------------------------------
    Step-1: prepare opt
    ------------------------------------
        ...
        for i, train_data in enumerate(train_loader):
            ...
            if current_step % opt['train']['checkpoint_test'] == 0 and opt['rank'] == 0:
                ...
                for test_data in test_loader:
                    ...
                    # util.mkdir(img_dir)
                    # print(test_data['L'].shape, test_data['H'].shape)
                    ...
                    E_tensor = E_tensor * 2. - 1.
                    H_tensor = H_tensor * 2. - 1.
                    ...
                    category = image_name_ext.split('-')[0].lower()
                    if category not in category_psnr:
                        category_psnr[category] = []
                    category_psnr[category].append(current_psnr)
                    if category not in category_ssim:
                        category_ssim[category] = []
                    category_ssim[category].append(current_ssim)
                    if category not in category_lpips:
                        category_lpips[category] = []
                    category_lpips[category].append(current_lpips)

                    logger.info('{:->4d}--> {:>10s} | PSNR: {:<6.4f}dB; SSIM: {:<6.4f}; LPIPS: {:<6.4f}'.format(idx, image_name_ext, current_psnr, current_ssim, current_lpips))
                ...
                averages = {}
                for category, psnr_list in category_psnr.items():
                    averages[category] = sum(psnr_list) / len(psnr_list)
                    logger.info('Average PSNR of {:>10s} is {:<4.2f}dB'.format(category, averages[category]))
                averages = {}
                for category, ssim_list in category_ssim.items():
                    averages[category] = sum(ssim_list) / len(ssim_list)
                    logger.info('        SSIM of {:>10s} is {:<4.2f}'.format(category, averages[category]))
                averages = {}
                for category, lpips_list in category_lpips.items():
                    averages[category] = sum(lpips_list) / len(lpips_list)
                    logger.info('       LPIPS of {:>10s} is {:<4.2f}'.format(category, averages[category]))
                ...
                # testing log
                logger.info('<epoch:{:3d}, iter:{:8,d}, Average PSNR : {:<6.4f}dB SSIM : {:<6.4f} LPIPS : {:<6.4f}\n'.format(epoch, current_step, avg_psnr, avg_ssim, avg_lpips))
