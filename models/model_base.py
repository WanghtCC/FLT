...
class ModelBase():
    def __init__(self, opt):
        ...
        self.device = torch.device('cuda' if opt['gpu_ids'] != [-1] else 'cpu')
    ...
    def describe_network(self, network):
        ...
        msg += 'Flops: {}'.format(network.flops() / 1e9) + ' GFLOPs\n'
        msg += 'Params number: {}'.format(sum(map(lambda x: x.numel(), network.parameters())) / 1e6) + ' M\n'
        return msg
    ...
    # ----------------------------------------
    def save_network(self, save_dir, network, network_label, iter_label, is_best):
        if is_best:
            save_filename = 'best_model_{}.pth'.format(network_label)
        else:
            save_filename = '{}_last_{}.pth'.format(iter_label, network_label)
    ...
    # ----------------------------------------
    def save_optimizer(self, save_dir, optimizer, optimizer_label, iter_label, is_best):
        if is_best:
            save_filename = 'best_model_{}.pth'.format(optimizer_label)
        else:
            save_filename = '{}_last_{}.pth'.format(iter_label, optimizer_label)
        save_path = os.path.join(save_dir, save_filename)
