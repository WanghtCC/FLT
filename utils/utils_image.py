...
def unsharp_mask(image, sigma=1.0, strength=1.5):
    """
    使用 Unsharp Masking 增强图像边缘对比度。
    """
    blurred = cv2.GaussianBlur(image, (0, 0), sigma)
    sharpened = cv2.addWeighted(image, 1 + strength, blurred, -strength, 0)
    return sharpened

def uniform_center_crop(image, crop_size, num_crops):
    """
    裁剪图像，裁剪中心在限定区域内均匀分布。
    """
    ...
    # 计算中心限定区域的范围
    center_x_min = crop_width // 2
    center_x_max = img_width - crop_width // 2
    center_y_min = crop_height // 2
    center_y_max = img_height - crop_height // 2

    # 生成均匀分布的裁剪中心
    centers = [
        (
            random.randint(center_x_min, center_x_max),
    ...
    for center_x, center_y in centers:
        ...
        crop = image[y_start:y_start + crop_height, x_start:x_start + crop_width]
        crops.append(crop)

    return crops

def process_image(image, target_size, output_dir, original_name):
    """
    处理单张图像，包括增强、下采样和裁剪。
    """
    img_height, img_width = image.shape[:2]
    target_width, target_height = target_size
    max_scale = min(img_width // target_width, img_height // target_height)

    # Step 1: Apply Unsharp Masking
    # enhanced_image = unsharp_mask(image)
    enhanced_image = image

    # Step 2: Multi-scale downsampling and cropping
    all_crops = []

    # Original
    original_img = cv2.resize(enhanced_image, target_size, interpolation=cv2.INTER_CUBIC)
    name, ext = os.path.splitext(original_name)
    output_path = os.path.join(output_dir, f"{name}_{0}_{0}{ext}")
    cv2.imwrite(output_path, original_img)

    # Original scale
    crops = uniform_center_crop(enhanced_image, target_size, num_crops=max_scale)

    # Save crops with naming rule
    for i, crop in enumerate(crops):
        crop_resized = cv2.resize(crop, target_size, interpolation=cv2.INTER_CUBIC)
        name, ext = os.path.splitext(original_name)
        output_path = os.path.join(output_dir, f"{name}_{1}_{i}{ext}")
        cv2.imwrite(output_path, crop_resized)

    # Downsampled scales
    for scale in range(2, max_scale + 1, 2):
        ...
        # Random cropping with uniformly distributed centers
        crops = uniform_center_crop(downsampled, target_size, num_crops=max(1, max_scale - scale))

        # Save crops with naming rule
        for i, crop in enumerate(crops):
            crop_resized = cv2.resize(crop, target_size, interpolation=cv2.INTER_CUBIC)
            name, ext = os.path.splitext(original_name)
            output_path = os.path.join(output_dir, f"{name}_{scale}_{i}{ext}")
            cv2.imwrite(output_path, crop_resized)

    print(f"Processed crops saved to {output_dir}/{original_name}.")
